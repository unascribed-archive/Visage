...
public class RenderThread extends Thread {
	...
	public void run() {
		try {
			Lapitar.log.info("Waiting for jobs");
			try {
				while (run) {
				...
				Lapitar.log.log(Level.SEVERE, "A fatal error has occurred in the render thread run loop.", e);
			}
		} catch (Exception e) {
			Lapitar.log.log(Level.SEVERE, "A fatal error has occurred while setting up a render thread.", e);
		}
	}
